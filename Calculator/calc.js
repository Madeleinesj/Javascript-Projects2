...
const Calculator = {
    ...
    Display_Value: '0',
    //This wil hold the first operand for any expressions, we sit it to null for now.
    First_Operand: null,
...
function Handle_Operator(Next_Operator) {
    ...
    //When an operator key is pressed we convert that current number
    //displayed on the screen to the number and then store the result in 
...
keys.addEventListener('click', (event) => {
    ...
    //that was clicked. 
    const { target } = event;
    //If the elemetn that was clicked o nis not a button, exit the function.
